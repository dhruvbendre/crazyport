...
from .horizon import chalk_star, glyph_memory, horizon_svg
...
def render_head(settings: Settings) -> None:
    ...
    st.markdown(
        ...
        <p class="mn-eyebrow">VIII · The archive</p>
        <h1 class="mn-name">{html.escape(settings.archive_name)}</h1>
...
def render_empty_state(settings: Settings, categories: list[tuple[str, str]], *, archive_empty: bool, note: str) -> str | None:
    ...
    for col, (label, question) in zip(cols, categories):
        with col:
            with st.container(key=f"chip-{label.lower()}"):
                if st.button(label, key=f"chip-btn-{label.lower()}", use_container_width=True):
                    picked = question
    if archive_empty:
...
def archive_container(index: int, *, mode: str):
    ...
    with container:
        mark = chalk_star(15) if mode in {"refusal", "error"} else glyph_memory(18)
        label = "the archive" if mode != "error" else "the archive · interrupted"
        st.markdown(
...
def thinking_placeholder():
    slot = st.empty()
    slot.markdown('<span class="mn-thinking">the archive is remembering<i></i><i></i><i></i></span>', unsafe_allow_html=True)
    return slot
